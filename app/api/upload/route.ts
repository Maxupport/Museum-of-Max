...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 1. 若環境變數包含 Vercel Blob Token (在 Vercel 啟用 Blob 後會自動注入)
    if (blobToken) {
    ...
    if (process.env.VERCEL) {
      return NextResponse.json({
        ok: false,
        error: '圖片上傳失敗：未找到 Vercel Blob Token。請至 Vercel Dashboard -> Storage -> Blob 點擊「Connect to Project」或在 Settings -> Environment Variables 確認存有 BLOB_READ_WRITE_TOKEN。',
      }, { status: 400 });
    }
