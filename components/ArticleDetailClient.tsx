...
import { ArrowLeft, Calendar, BookOpen, Share2, Check, Sparkles } from 'lucide-react';
...
interface ArticleDetailClientProps {
  initialArticle: SerializedArticleData | null;
  exhibitId: string;
  articleId: string;
}
...
export default function ArticleDetailClient({
  ...
  const articleData = dbArticle ? {
    ...
    author: 'Maxupport Curator',
    content: [
      { type: 'heading', text: '一、前言：開啟個人聲音覺察之旅' },
      { type: 'paragraph', text: '聲音不僅是傳遞語意與文字的工具，更是個人情緒、氣場與內在狀態的直接延伸。在這次的個人聲音探索實驗中，我嘗試透過呼吸調整、發聲共鳴位移與日常對話記錄，重新認識屬於自己的真實聲響。' },
      { type: 'quote', text: '「每一次發聲，都是身心狀態最誠實的鏡像反映。」' },
      { type: 'heading', text: '二、圖文紀錄：共鳴腔體與呼吸控氣訓練' },
      { type: 'paragraph', text: '透過胸腔與鼻腔共鳴的調控，能顯著提升語調的圓潤度與穩定度。以下為聲音訓練過程中的現場空間與感官覺察紀錄：' },
      { type: 'image', url: 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?auto=format&fit=crop&w=1200&q=80', caption: '圖 1：聲音探索實驗室與發聲測試設備記錄' },
      { type: 'heading', text: '三、影音範例與精選段落演示' },
      { type: 'paragraph', text: '下方為聲音探索練習時錄製的影音段落，展示語調層次與氣息銜接的微幅轉折：' },
      { type: 'video', url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', caption: '影片 1：個人聲音共鳴與朗讀語調測試範例' },
      { type: 'heading', text: '四、結語與 Notion 專題同步規劃' },
      { type: 'paragraph', text: '本專區未來將持續與 Notion 資料庫即時連動，隨時補充全新的聲音探索日記、圖文紀錄與影音音軌。' },
    ],
  } : null;
    ...
    <div style={{ padding: '3.5rem 1.5rem 6rem', maxWidth: '820px', margin: '0 auto', minHeight: '100vh', boxSizing: 'border-box' }}>
      ...
      {vocalCourseData ? (
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontWeight: 600, color: '#fff', marginBottom: '0.2rem' }}>
                🎙️ 人聲優化演進時間軸（共 {vocalCourseData.versions.length} 個演進版本錄音）
              </div>
        ...
        <article className="animate-fade-in" style={{ fontSize: '1.05rem', lineHeight: 1.9, color: 'rgba(255,255,255,0.85)' }}>
          ...
          {/* AI 引用友善提示區 (How to cite this article) */}
          <div style={{
            marginTop: '4rem',
            padding: '1.5rem',
